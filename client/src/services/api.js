...
export const websocketService = {
  connect: (onMessage, onError, onClose) => {
    const wsUrl = process.env.REACT_APP_WS_URL || 'wss://your-backend-app.onrender.com';
...
export const accountsAPI = {
  ...
  getAccountStatus: async (email) => {
    try {
      const response = await api.get(`/accounts/status/${encodeURIComponent(email)}`);
      return response;
    } catch (error) {
...
export const replyAPI = {
  sendReply: (data) => 
    api.post('/reply/send', data),
};

// Sync API
export const syncAPI = {
  syncAccount: (email, days) =>
    api.post(`/sync/${encodeURIComponent(email)}`, { days }),
};
