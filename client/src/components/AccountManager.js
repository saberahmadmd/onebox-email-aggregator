...
const AccountManager = ({ onAccountChange }) => {
  ...
  const startAccountPolling = (email) => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (newAccount && newAccount.status === 'connected') {
          console.log(`✅ Account ${email} is now connected`);
          clearInterval(interval);
          
          // Remove from polling intervals
          setPollingIntervals(prev => {
            const newIntervals = { ...prev };
            delete newIntervals[email];
            return newIntervals;
          });
          
          alert(`✅ Account ${email} connected successfully!`);
    ...
    setTimeout(() => {
      if (pollingIntervals[email]) {
        clearInterval(pollingIntervals[email]);
        setPollingIntervals(prev => {
          const newIntervals = { ...prev };
          delete newIntervals[email];
          return newIntervals;
        });
        console.log(`⏰ Stopped polling for ${email} after 3 minutes`);
      }
    }, 180000);
  };
  ...
  const handleAddAccount = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        startAccountPolling(formData.email);
      ...
      // If it's a timeout, it might still be processing in background
      if (error.message.includes('timeout') || error.code === 'ECONNABORTED') {
        alert('⏳ Account addition is taking longer than expected. The account is being added in the background. Check the accounts list in a few minutes.');
        
        // Start polling for this account anyway
        startAccountPolling(formData.email);
      } else {
        alert(errorMessage);
      }
  ...
  const handleRemoveAccount = async (email) => {
    if (window.confirm(`Remove ${email}? This will also remove all associated emails from the system.`)) {
      try {
        // Clear any polling for this account
        if (pollingIntervals[email]) {
          clearInterval(pollingIntervals[email]);
          setPollingIntervals(prev => {
            const newIntervals = { ...prev };
  ...
  const syncAccountEmails = async (email) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://your-backend-app.onrender.com/api'}/sync/${encodeURIComponent(email)}`, {
        method: 'POST',
        headers: {
  ...
  const isAccountConnecting = (email) => {
    return pollingIntervals[email] !== undefined;
  };
            ...
            <div key={account.email} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {isAccountConnecting(account.email) && (
              ...
              <div className="flex space-x-2 ml-4">
                <button
                  ...
                  disabled={getSyncStatus(account.email) || account.status !== 'connected'}
